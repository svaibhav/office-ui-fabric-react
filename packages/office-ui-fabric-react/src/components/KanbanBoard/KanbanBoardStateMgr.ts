...
export class KanbanBoardStateMgr {
  ...
  public getLaneItems = (laneColumn: ILaneColumn) => {
    if (!this._lanes.get(laneColumn.name)) {
      this._lanes.set(laneColumn.name.toString(), this._items.filter(item => item.population === laneColumn.name));
    }
    // return this._items.filter(item => item.color === laneColumn.name);
    return this._lanes.get(laneColumn.name).slice();
  };
  ...
  public moveItem = (item: any, sourceCol: any, destinationCol: any, sourceInx: any, destInx: any): void => {
    if (sourceCol == destinationCol) {
      ...
      item.population = destinationCol.toUpperCase();
